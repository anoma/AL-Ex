defmodule Examples.ALFunctor do
  @moduledoc """
  I provide `vm_functor/3` and `call_term/1` examples. `vm_functor` is Prolog's
  ...
  example decomposes_a_ground_tuple() do
    {:atomic, {bindings, _state}} =
      run branch: :examples do
        vm_functor(term, :foo, [1, 2])
        vm_functor(term, name, args)
      end
  ...
  example call_term_fails_with_no_receiver() do
    {:aborted, _} =
      run branch: :examples do
        vm_functor(term, :does_not_understand, [])
        call_term(term)
      end
